...
class Users(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)  # A unique ID for each user, used as the primary key, indexed for faster query performance
...
class Todos(Base):
    ...
    complete = Column(Boolean, default=False)  #If not complete value is provided this will default to False (0)
    owner_id = Column(Integer, ForeignKey("users.id"))  # Links each todo to a user in the Users table.
